...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        movies = Movie.objects.filter(status='NP')
        for mv in movies:
                ...
                if result.status_code == 200:
                    ...
                    for shows in soup.find_all("div", "show-movies"):
                        ...
                        for st in show_times:
                            ...
                            Showtime.objects.create(date=datetime.datetime.utcnow() + datetime.timedelta(days=i),
                                                    movie=mv, cinema=Showtime.get_cinemahall_code(show_hall_name),
                                                    time=time, booking_url=booking_url)
                    i += 1
